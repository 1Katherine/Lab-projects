...
parser.add_argument('--benchmark', type=str, help='type of benchmark')
parser.add_argument('--group', type=int, default = 30, help='number of group')
parser.add_argument('--niters', type=int, default = 10, help='The number of iterations of the GA algorithm')
args = parser.parse_args()
print('benchmark=' + str(args.benchmark) + ', group = ' + str(args.group) + ', niters = ' + str(args.niters))

# 服务器运行spark时config文件
config_run_path = "/usr/local/home/yyq/ga/config/" + args.benchmark + "/"
# 重要参数
vital_params_path = "/usr/local/home/yyq/ga/parameters_set.txt"
# 维护的参数-范围表
conf_range_table = "/usr/local/home/yyq/ga/Spark_conf_range_" + args.benchmark.split('-')[0] + ".xlsx"
# 保存的每一代的最优参数
generation_confs = "/usr/local/home/yyq/ga/generationBestConf.csv"
# 保存 GA的参数
ga_confs_path = "/usr/local/home/yyq/ga/gaConfs.csv"
# 保存所有的 Y
all_history_Y_save_path = '/usr/local/home/yyq/ga/all_history_y.csv'
generation_best_pic_directory = '/usr/local/home/yyq/ga/target.png'
...
def run(configNum):
    # configNum = None
    # 使用给定配置运行spark
    run_cmd = '/usr/local/home/zwr/' + args.benchmark + '-ga.sh ' + str(configNum) + ' /usr/local/home/yyq/ga/config/' + args.benchmark
    os.system(run_cmd)
...
sizePop = args.group    # 种群数量
maxIter = args.niters    # 迭代次数
probMut = 0.01  # 变异概率
# 调用遗传算法，记录整个搜索时间
...
ax[0].set_title('binary GA')
Y_history.min(axis=1).cummin().plot(kind='line')
plt.savefig(generation_best_pic_directory)
plt.show()
