    ...
    def lhs(self):
        ...
        for i in range(self.D):
            for j in range(self.N):
                # if self.precisions[i] == 1.0:
                #     temp[j] = round(np.random.uniform(low=j * d, high=(j + 1) * d, size=1)[0])
                # if self.precisions[i] == 0.01:
                #     temp[j] = round(np.random.uniform(low=j * d, high=(j + 1) * d, size=1)[0], 2)
