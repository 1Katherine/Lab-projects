...
class Choose:
    ...
    def process_data(self,features,index):
        ...
        x_train, x_test, y_train, y_test = train_test_split(features_data, target_data, test_size=0.25, random_state=22+index*3)
    ...
    def choose_features(self,features):
        ...
        if len(features_with_importance) >= self.left_num:
            if len(features_with_importance)<100:
                self.step= 5
            if len(features_with_importance)<50:
                self.step= 3
            # print("进入")
    ...
    def plot_error(self):
        ...
        plt.tight_layout()
        plt.savefig(fname=self.save_path+self.name + "_"+self.prefix+"_error.png", figsize=[10, 10])
        plt.show()
        plt.clf()
        plt.xlabel("特征数量")
        plt.ylabel("误差")
