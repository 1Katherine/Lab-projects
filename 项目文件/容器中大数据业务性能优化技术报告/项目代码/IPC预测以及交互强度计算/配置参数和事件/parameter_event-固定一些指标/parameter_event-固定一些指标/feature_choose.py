...
class Choose:
    def __init__(self,name,features,data,step,prefix,save_path):
        self.name=name
    ...
    def process_data(self,features):
        ...
        target_data = self.data["instructions"]
    ...
    def plot_error(self):
        ...
        plt.tight_layout()
        plt.savefig(self.save_path+self.name + "_"+self.prefix+"_error.png")
        plt.show()
        plt.clf()
    ...
    def plot_feature_importance(self):
        ...
        self.sort_final_features=features_
        ...
        plt.savefig(self.save_path+self.name + "_"+self.prefix+"_features.png")
        plt.show()
        plt.clf()
    ...
    def error_to_file(self,str_time):
        #将误差最低时的特征记录下来
        fileObject=open(self.save_path+self.prefix+"_error.txt",mode="a+")
        fileObject.write('\n')
        fileObject.write(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
        fileObject.write('\n')
        fileObject.write("训练时长")
