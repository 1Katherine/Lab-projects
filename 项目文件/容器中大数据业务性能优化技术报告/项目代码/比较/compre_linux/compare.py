...
class Compare_Yarn_K8s:
        ...
        for index in range(1, self.num + 1):
            ...
            for feature in self.intersection_features:
                if feature in features_cur:
                    ...
                    # print(feature)
                    plt.savefig(self.save_path + "line_plots" + "/" + feature + "_" + str(index) + ".png")
                    plt.clf()
                else:
    ...
    def calculate(self):
        ...
        # 画柱状图
        def draw_diff_bar(title, diff_dict, path):
            ...
            features_list=diff_dict.keys()
            values_list=diff_dict.values()
            """
            ...
            def autolable(rects):
                for rect in rects:
                    ...
                    if height >= 0.7:
                        plt.text(rect.get_x() + rect.get_width() / 2.0 - 0.3, height + 0.05, '%.3f' % height, size=20)
                    elif height <= 0.3:
                        plt.text(rect.get_x() + rect.get_width() / 2.0 - 0.3, height + 0.05, '%.3f' % height, size=20)
            ...
            plt.title(title, fontdict={'family': 'Times New Roman', 'size': 36})
            plt.yticks(np.arange(0, 1.05, 0.1), fontproperties='Times New Roman', size=36)
            plt.xticks(rotation=270)
            plt.xticks(list(range(0,len(features_list), 1.5)), features_list)
            colors = []
            ...
            ax = plt.bar(features_list, values_list, color=colors,edgecolor='black', width=0.6)  # edgecolor边框颜色
            ...
            plt.savefig(self.save_path + "bar_plots" + "/" + path + "/" + "diff" + "/" + str(index) + ".png")
        ...
        for index in range(1, self.num + 1):
            ...
            draw_diff_bar(title="mean_k8s/(mean_k8s+mean_yarn)", diff_dict=mean_diff_dict, path="mean")
        ...
        # 记录该指标有多少次是k8s比yarn大
        def draw_count_bar(count_dict, name):
            plt.figure(figsize=(60, 40), dpi=200)
            plt.bar(count_dict.keys(), count_dict.values(),width=1)
            plt.yticks(np.arange(0, 51, 5), fontproperties='Times New Roman', size=36)
            plt.xticks(rotation=270, size=25)
            plt.savefig(self.save_path + name + ".png")
            plt.clf()

        draw_count_bar(mean_count_dict, "mean_count")
        draw_count_bar(median_count_dict, "median_count")
    ...
    def config_subplot(self):
        ...
        for feature in self.intersection_features:
            ...
            for index in range(1, self.num + 1):
                ...
                plt.savefig(self.save_path + "sub_plots" + "/" + new_feature + ".png")
            plt.clf()
    ...
    def get_Mean_Meadian_info(self):
        ...
        def draw_feature_count_bar(new_feature, count_data, path):
            ...
            plt.savefig(self.save_path + "bar_plots" + "/" + path + "/" + new_feature + ".png")
            plt.yticks()
            plt.figure(figsize=(60, 40), dpi=220)
            plt.clf()
