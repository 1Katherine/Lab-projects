...
for name in ["lgb", "rf", "ada", "gdbt", "xgb"]:
    if not os.path.isdir(save_path + name):
    ...
    model_construct_Obj= time_predict.Model_construct(name=name, save_path=new_save_path, target=target, data=data, features=features_list)
