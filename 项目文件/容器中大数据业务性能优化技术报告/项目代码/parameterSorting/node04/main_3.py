...
# import parameters_interact   #用于计算交互强度的类
...
parser.add_argument('-p','--prefix',help='the prefix of file,it represents which kind of train data')
...
#prefix默认为parameters
total_choose=parameter_choose.Choose(name=name,features=features_list,step=step,prefix=prefix,data=data,save_path=save_path,target=target,left_num=left_num)
total_choose.main()
...
for i in range(len(total_choose.final_importance)):
    ...
    if cur_sum>=sum_importance*0.85:
        break
...
interact_obj=parameters_interact.Interaction_calculate(data=data,final_features=total_choose.final_features,
