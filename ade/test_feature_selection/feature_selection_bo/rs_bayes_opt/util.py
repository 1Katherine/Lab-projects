...
def acq_max(ac, gp, y_max, Tconstraint, bounds, random_state, n_warmup=10000, n_iter=10):
    ...
    # ys = ac(x=x_tries, gp=gp, y_max=y_max)
    # x_max = x_tries[ys.argmax()]
    # max_acq = ys.max()
    # print('upper = \n' + str(ys))
    # print('upper.argmax() = ' + str(ys.argmax()))


    # Explore the parameter space more throughly
    # x_seeds = random_state.uniform(bounds[:, 0], bounds[:, 1],
    #                                size=(n_iter, bounds.shape[0]))
    # x_seeds = standardization(x_seeds)
    ...
    #         print('-res.fun[0] = ' + str(-res.fun[0]) + 'max_acq = ' + str(max_acq))
    #         x_max = res.x
    #         max_acq = -res.fun[0]
