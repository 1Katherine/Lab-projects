...
k = 8
# 调用代码：python feature_selection_bo_rfcorr_laptop.py --sampleType=all --ganrsGroup=4 --niters=10 --initFile=/usr/local/home/yyq/bo/ganrs_bo/wordcount-100G-GAN.csv
import argparse
parser = argparse.ArgumentParser(description='manual to this script')
parser.add_argument('--benchmark', type=str, default='wordcount-100G', help='benchmark type')
parser.add_argument('--niters', type=int, default=2, help='The number of iterations of the Bayesian optimization algorithm')
parser.add_argument('--ninits', type=int, default=2, help='The number of initsamples of the Bayesian optimization algorithm')
args = parser.parse_args()
print('总共搜索的样本数niters = ' + str(args.niters) + '\t 第一次随机采样的初始样本数 ninits = ' + str(args.ninits))
# 获取当前文件路径
...
def black_box_function(**params):
    ...
    for conf in vital_params['vital_params']:
        ...
        print('-schafferRun(i)中的config为 : ' + str())
...
if __name__ == '__main__':
    ...
    while current_niterations + 5 <= max_niterations:
        ...
        if iterations == 1:
            ...
            if len(sorted(d2)) < 2:
                print('剩余参数少于2个不再降维直接搜索。降维后剩余的参数个数:' + str(len(d2)) + '降维后的vital_params_range = ' + str(d2))
                ...
                res_samples_df, vitual_params = init_bo(init_samples, iter = 3, vital_params_range = d2)
                iterations += 1
                current_niterations += 5
        print('第 ' + str(iterations) + ' 次迭代的结果样本为\n' + str(res_samples_df))
