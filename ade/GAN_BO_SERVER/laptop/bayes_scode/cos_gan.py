...
def train(df, first_time, args):
    ...
    for epoch in range(args.epochs):
        ...
        for n, (real_samples, _) in enumerate(train_loader):
            ...
            计算其余弦值
            '''
            feature1=generated_samples.view(generated_samples.shape[0],-1)
            # print(feature1)
            feature2=real_samples.view(real_samples.shape[0],-1)
            # print(feature2)
            feature1=F.normalize(feature1)
            # print(feature1)
            feature2=F.normalize(feature2)
            # print(feature2)
            loss_generator = -1*feature1.mm(feature2.t().float())
            loss_generator.backward()
            optimizer_generator.step()
            # Show loss
            if epoch % 1 == 0:
        ...
        对早停条件进行变更
        '''
        if loss_generator < -0.99:
            m = m + 1
        # if distance < args.number_features * 0.25:
        #     m = m + 1
    ...
    while (len(final_data) < 100):
        ...
        for i in range(samples):
            ...
            generated_samples['distance'].iat[i]=generated_samples['cos_distance'].iat[i]/generated_samples['Euclidean_distance'].iat[i]
        '''
        2022/1/13
        ...
        for indexs, row in generated_samples.iteritems():
            ...
            generated_samples[indexs] = sparkConfRangeDf.loc[indexs, 'min'] + ((generated_samples[indexs] - a) *
                                                                               (sparkConfRangeDf.loc[indexs, 'max'] -
                                                                                sparkConfRangeDf.loc[indexs, 'min'])) / (b - a)
            try:
                if (df['spark.memory.offHeap.size'].item() == 0):
        ...
        for i in range(samples):
            for j in range(number_Features):
                ...
                if(df.iloc[0, j] != 0):
                    generated_samples['dissimilarity_value'].iat[i]=generated_samples['dissimilarity_value'].iat[i]+abs((df.iloc[0, j]-generated_samples.iloc[i, j])/df.iloc[0, j])
        ...
        print(generated_samples.loc[:, ['range', 'dissimilarity_value','cos_distance','Euclidean_distance','distance']])
        '''
        2022/2/16
        筛选出合法值,误差不能超过20%
        '''
    ...
    print(final_data.loc[:, ['range', 'dissimilarity_value','cos_distance','Euclidean_distance','distance']])
    final_data.to_csv('../data_analysis/cosgan_sample.csv', index=None,mode='a')
    '''
    2022/1/20
    根据相似程度选择合适的样本点 0
    '''
