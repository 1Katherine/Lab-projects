...
class BayesianOptimization(Observable):
    ...
    def _prime_queue(self, init_points):
        ...
        import numpy as np
        print('初始样本shape = ' + str(self.custom_initsamples.shape))
        for sample in self.custom_initsamples:
            #target = 0 - sample[-1:][0]
            # 正转负
            target = 0 - sample[-1]
            print('custom_initsamples中的sample为 : ' + str(sample) + '\tsample执行时间为 : ' + str(target))   
            if int(target) == -100000:
                print('该样本运行失败，不能作为bo的初始样本 , x = ' + str(sample[:-1]))
            else:
                print('该样本运行成功，将其作为bo的初始样本')
                x = sample[:-1]
                self.register(x, target)
        print('参数空间中的数据：self._space.params = ' + str(self._space.params) +  '\nself._space.target = ' + str(self._space.target))
