...
def random_for_gan(n):
    ...
    for i in range(n):
        ...
        config.append(y)
        print('随机生成的第' + str(i+1) +'配置:' + str(config) + ' . 随机生成的配置和实际运行时间:' + str(config))
...
def gan_random(n, type, bo_res_df, iterations):
    base = args.initpoints
    print('gan_random方法获取的参数:' + '\t base = ' + str(base) + '\t n = ' + str(n) + '\t type = ' + str(type) + '\t bo_res_df = \n' + str(bo_res_df) + '\niterations = ' + str(iterations))
    ...
    print('GAN的训练样本个数和生成样本个数都为 = ' + str(base // 2) + ' ， GAN使用的训练样本为\n' + str(train_df_all))
    # ------------------------- 第一轮gan：gan拿随机样本中最好的一个做训练，再从gan生成的9个假样本中取执行时间最短的两个样本 start ----------------------------
    bestconfig = train_df_all.iloc[:1,:-1]
    print('train_df_all中的bestconfig为' + str(bestconfig))
    generate_data=train(bestconfig, first_time, args)
    print('GAN网络的生成数据generate_data为' + str(generate_data))
...
if __name__ == '__main__':
    ...
    while iterations < maxIters:
        ...
        if iterations == 0:
            ...
            print('iterations = ' + str(iterations) + '\t 传给bo的初始样本initsamples为 \n' + str(initsamples))
            #定义贝叶斯优化模型
