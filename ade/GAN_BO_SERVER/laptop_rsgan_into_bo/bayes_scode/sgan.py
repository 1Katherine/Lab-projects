...
def train(df, first_time, args):
    ...
    while (len(final_data) < 100):
        ...
        for i in range(samples):
            generated_samples.loc[i,['cos_distance']] = cos_sim(generated_samples.iloc[i:i + 1, :df.shape[1]],
                                                               datasets)
            generated_samples.loc[i,['Euclidean_distance']]  = ed(generated_samples.iloc[i:i + 1, :df.shape[1]], datasets)
            generated_samples.loc[i,['distance']] = generated_samples['cos_distance'].iat[i] / \
    ...
    print(final_data.loc[:, ['range', 'dissimilarity_value', 'cos_distance', 'Euclidean_distance', 'distance']])
    final_data.to_csv(father_path + '/data_analysis/sgan_sample.csv', index=None, mode='a')
    '''
    2022/1/20
    根据相似程度选择合适的样本点 0
    '''
